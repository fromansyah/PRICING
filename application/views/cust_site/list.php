...
<body>
    <div class="container">
        <span width="100" style="background-color: #e0e0e0; display: block;">
            &nbsp;<font size="5" style="font-weight: bold; color: #737373">Customer Site : [<?echo str_replace('slash', '/', $cust_id)?>] <?echo $cust_name?></font>
        </span>
        <br/>
        <button class="btn btn-warning" onclick="back_to_cust()"><i class="glyphicon glyphicon-backward"></i> Back</button>
...
function add_site()
{
    ...
    $('[name="siteId"]').attr("readOnly", false);
    $('[name="siteId"]').val(<?=$cust_id?>);
...
function edit_site(id)
{
    ...
    $('[name="siteId"]').attr("readOnly", true);
...
//    $('.modal-title').text('Edit Product Price'); // Set title to Bootstrap modal title
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            $('[name="siteId"]').val(data.site_id);
            $('[name="custId"]').val(data.cust_id);
            $('[name="note"]').val(data.note);
            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Edit Customer Site'); // Set title to Bootstrap modal title
...
function delete_site(id, site)
{
    if(confirm('Are you sure delete this data: ' + site + ' ?'))
    {
        ...
        <div class="modal-content">
            ...
            <div class="modal-body form">
                <form action="#" id="form" class="form-horizontal">
                    <input type="hidden" value="<?=str_replace('slash', '/', $cust_id)?>" name="custId"/> 
                    <div class="form-body">
                        <div class="form-group">
                            <label class="control-label col-md-3">ID</label>
                            <div class="col-md-9">
                                <input name="siteId" placeholder="ID" class="form-control" type="text">
